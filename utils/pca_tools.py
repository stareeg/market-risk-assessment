"""
Инструменты сокращения размерности (п. 2.a) — PCA и факторный анализ.
...
Две основные задачи:
  ...
  * PCA/факторный анализ доходностей акций по КОРРЕЛЯЦИЯМ (ряды стандартизуются,
    т.к. волатильности разные): первая компонента — «рыночный» фактор.
"""
...
def factor_analysis(returns: pd.DataFrame, n_factors: int = 3):
    """
    Факторный анализ доходностей (альтернатива PCA из п. 2.a). Ряды стандартизуются.
